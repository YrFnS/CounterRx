...
- [x] W3.2 Patient delivery module.
...
- [x] W3.8 NDC live lookup (openFDA/RxNorm).
  CHECK: `grep -n 'lookupNdc\|counterrx:ndc-cache:v1\|ndcLiveLookup' src/lib/ndc.ts src/views/Inventory.tsx src/views/Settings.tsx src/data.ts`
  EXPECT: `src/lib/ndc.ts` `lookupNdc(ndc, { live })` hits `https://api.fda.gov/drug/ndc.json?search=product_ndc:"<10-digit>"` (strip dashes, pad to 10), returns `{name, genericName, manufacturer, route, activeIngredients}`; 200ms throttle + 5s timeout; on failure/skip falls back to `NDC_DIRECTORY` local; memory Map + localStorage `counterrx:ndc-cache:v1` (TTL 30d). Inventory Add product shows a "Look up" button (→ fills fields) with an "Using offline directory" hint when the fallback fired; `settings.ndcLiveLookup` (default true) toggles live off. i18n keys in both en.json and ar.json.
  EVIDENCE: `src/lib/ndc.ts` (lookup + cache + throttle + `clearNdcCache` test seam); `src/__tests__/ndc-lookup.test.ts` (10 cases: normalization 9→10/dashes, cache TTL hit/miss, fallback on fetch failure + empty results + live:false, throttle ≥200ms); Inventory `AddProductModal` wired to `lookupNdc` + offline hint; Settings `ProfileTab` adds the `ndcLiveLookup` toggle. Gate run: typecheck clean, 187 tests pass, build OK.
