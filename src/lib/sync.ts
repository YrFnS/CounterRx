...
function deliveryFrom(row: Row): Delivery {
  return {
    ...
    mode: text(row, "mode", "delivery") as Delivery["mode"],
    status: text(row, "status", "queued") as Delivery["status"],
    driver: optionalText(row, "driver"),
    scheduledAt: rowEpoch(row, "scheduled_at"),
    proof: optionalText(row, "proof"),
    createdAt: rowEpoch(row, "created_at"),
  };
}
...
function settingsFrom(
  ...
  return {
    orgName: text(row, "org_name", fallback.orgName),
    branch: text(row, "branch", fallback.branch),
    address: text(row, "address", fallback.address),
    phone: text(row, "phone", fallback.phone),
    license: text(row, "license", fallback.license),
    currency: text(row, "currency", fallback.currency),
    receiptFooter: text(row, "receipt_footer", fallback.receiptFooter),
    receiptTerms: text(row, "receipt_terms", fallback.receiptTerms),
    showBarcode: booleanValue(row, "show_barcode", fallback.showBarcode),
    loyalty: loyalty as OrgSettings["loyalty"],
    scanBeep: booleanValue(row, "scan_beep", fallback.scanBeep),
    idleLockMins: numberValue(row, "idle_lock_mins", fallback.idleLockMins),
    autoSnapshotMins: numberValue(
      row,
      "auto_snapshot_mins",
      fallback.autoSnapshotMins,
    ),
    terminalId: text(row, "terminal_id", fallback.terminalId),
    hardwareEnabled: booleanValue(
      row,
      "hardware_enabled",
      fallback.hardwareEnabled,
    ),
    savedReportViews: Array.isArray(rawViews)
      ? (rawViews as OrgSettings["savedReportViews"])
      : fallback.savedReportViews,
  };
}
...
export function rowsFor(data: BackendData): Record<TableName, Row[]> {
  return {
    ...
    backorders: data.backorders.map((b) => ({
      id: b.id,
      patient: b.patient,
      ...
      eta_days: b.etaDays,
      supplier: b.supplier,
      arrived_at: nullable(b.arrivedAt),
      notified_at: nullable(b.notifiedAt),
    })),
    rx_transfers: data.rxTransfers.map((r) => ({
      id: r.id,
      transfer_no: r.transferNo,
      direction: r.direction,
      prescription_id: nullable(r.prescriptionId),
      patient: r.patient,
      drug: r.drug,
      qty: r.qty,
      other_pharmacy: r.otherPharmacy,
      other_phone: nullable(r.otherPhone),
      prescriber: r.prescriber,
      refills_remaining: r.refillsRemaining,
    ...
    suppliers: data.suppliers.map((s) => ({
      id: s.id,
      name: s.name,
      contact: nullable(s.contact),
      phone: nullable(s.phone),
      email: nullable(s.email),
      terms: s.terms,
      lead_days: s.leadDays,
      min_order: s.minOrder,
      price_book: s.priceBook ?? [],
      archived: s.archived ?? false,
    })),
    purchase_orders: data.purchaseOrders.map((p) => ({
      id: p.id,
      supplier_id: nullable(p.supplierId),
      lines: p.lines,
      status: p.status,
      created_at: p.createdAt,
      expected_at: p.expectedAt,
      received_at: nullable(p.receivedAt),
      invoice_id: nullable(p.invoiceId),
      note: nullable(p.note),
    })),
    ap_invoices: data.apInvoices.map((i) => ({
      id: i.id,
      number: i.number,
      supplier_id: nullable(i.supplierId),
      po_id: nullable(i.poId),
      date: i.date,
      due_days: i.dueDays,
      total: i.total,
      payments: i.payments,
    ...
    deliveries: data.deliveries.map((d) => ({
      ...
      mode: d.mode,
      status: d.status,
      driver: nullable(d.driver),
      scheduled_at: d.scheduledAt,
      proof: nullable(d.proof),
      created_at: d.createdAt,
    })),
    web_orders: data.webOrders.map((o) => ({
      id: o.id,
      ...
      decline_reason: nullable(o.declineReason),
      created_at: o.createdAt,
    })),
    time_entries: data.timeEntries.map((t) => ({
      id: t.id,
      staff_id: nullable(t.staffId),
      in_at: t.inAt,
      out_at: nullable(t.outAt),
    })),
    staff: data.staff.map((s) => ({
      ...
      initials: s.initials,
      active: s.active,
      created_at: timestamp(s.createdAt),
    })),
    settings: [
      {
        id: 1,
        org_name: data.settings.orgName,
        ...
        receipt_terms: data.settings.receiptTerms,
        show_barcode: data.settings.showBarcode,
        loyalty: {
          ...data.settings.loyalty,
          savedReportViews: data.settings.savedReportViews,
        },
        scan_beep: data.settings.scanBeep,
        idle_lock_mins: data.settings.idleLockMins,
        auto_snapshot_mins: data.settings.autoSnapshotMins,
        terminal_id: data.settings.terminalId,
        hardware_enabled: data.settings.hardwareEnabled,
      },
    ],
    restricted_log: data.restrictedLog.map((r) => ({
      id: r.id,
      at: r.at,
      product_id: nullable(r.productId),
      qty: r.qty,
    ...
    shifts: data.shifts.map((s) => ({
      ...
      cashier_id: nullable(s.cashierId),
      cashier_name: s.cashierName,
      opened_at: s.openedAt,
      closed_at: nullable(s.closedAt),
      status: s.status,
      opening_balance: s.openingBalance,
      closing_balance: nullable(s.closingBalance),
      counted_cash: nullable(s.countedCash),
      transactions: s.transactions,
      cash_movements: s.cashMovements,
      sales_total: s.salesTotal,
      refunds_total: s.refundsTotal,
      card_total: s.cardTotal,
      insurance_total: s.insuranceTotal,
      store_credit_total: s.storeCreditTotal,
      paid_in_total: s.paidInTotal,
      paid_out_total: s.paidOutTotal,
      expected_cash: s.expectedCash,
      over_short: nullable(s.overShort),
      notes: nullable(s.notes),
    })),
    store_credits: data.storeCredits.map((c) => ({
      id: c.id,
      customer_id: nullable(c.customerId),
      balance: c.balance,
      issued_at: c.issuedAt,
      expires_at: nullable(c.expiresAt),
      code: nullable(c.code),
    ...
    interaction_pairs: data.interactionPairs.map((i) => ({
      a: i.a,
      b: i.b,
      severity: i.severity,
      effect: i.effect,
    ...
    cold_chain_log: data.coldChainLog.map((c) => ({
      id: c.id,
      product_id: c.productId,
      temp_c: c.tempC,
      in_range: c.inRange,
      staff: nullable(c.staff),
      note: nullable(c.note),
      created_at: timestamp(c.at),
    })),
