...
function stateWithCart(lines: { productId: string; qty: number; lineDiscount?: { mode: "amt" | "pct"; value: number } }[]): State {
  return { ...seed(), redeemPoints: 0, cart: lines } as unknown as State;
}
...
describe("discounts (P3)", () => {
  ...
  it("stacks line discounts with invoice % and capped invoice $", () => {
    ...
    const t = cartTotals(state, 20, false, 0, 9999);
