...
interface State {
  ...
  online: boolean;
  /** Number of mutations currently held in the offline outbox (badges the banner). */
  outboxCount: number;
  /** Conflicts resolved against a newer server version since the last dismiss. */
  ...
  cart: {
    ...
    uom?: string;
    lineDiscount?: { mode: "amt" | "pct"; value: number };
    rxId?: string;
  }[];
  held: HeldSale[];
...
type Action =
  ...
  | {
      type: "UPDATE_STAFF";
      id: string;
      patch: Partial<Pick<Staff, "name" | "role" | "active">>;
    }
  ...
  | {
      type: "TOGGLE_RESTRICTED";
      productId: string;
      restricted: { limitPerSale: number } | undefined;
    }
  | {
      type: "UPDATE_SETTINGS";
      patch: Partial<Omit<OrgSettings, "loyalty">> & {
  ...
  | {
      ...
      taxExempt: boolean;
      idChecked: boolean;
      restricted?: { purchaser: string; idType: string; idLast4: string };
      couponDiscount?: number;
      invoiceDiscountAmt?: number;
      approvedBy?: string;
    }
  | {
      type: "SETTLE_PAY_LATER";
  ...
  | {
      ...
      productId: string;
      uom?: string;
      discount?: { mode: "amt" | "pct"; value: number };
      approvedBy?: string;
    }
  | {
      type: "SET_BATCH_PRICE";
      productId: string;
      batch: string;
      price: number | null;
    }
  ...
  | { type: "SET_OUTBOX_COUNT"; count: number }
  | { type: "ADD_CONFLICTS"; conflicts: ConflictReport[] }
  | { type: "KEEP_LOCAL_CONFLICT"; table: string; key: string }
  ...
  | {
      type: "PO_RECEIVE";
      poId: string;
      receipts: { productId: string; qty: number; expiry: string }[];
    }
...
> => {
  ...
  transactions.forEach((t, i) => {
    if (!t.refundOf && i % 3 === 0)
      t.customerId = customers[i % customers.length].id;
  });
  return {
    products,
    transactions,
    ...
    if (raw) {
      ...
      if (
        ...
        return {
          ...
          purchaseOrders:
            saved.purchaseOrders ?? makePurchaseOrders(Date.now()),
...
export function cartTotals(
  ...
  const lines: TxLine[] = state.cart.map((c) => {
    ...
    const overridden =
      ...
      c.priceOverride > 0 &&
      c.priceOverride !== base;
    const kitSummary =
      p.kit && p.kit.length > 0
        ? p.kit
            .map(
              (k) =>
                `${k.qty}× ${state.products.find((x) => x.id === k.productId)?.name ?? k.productId}`,
            )
            .join(" + ")
        : undefined;
    return {
      ...
      daw: c.daw,
      substituted: c.substitutedFrom
        ? state.products.find((x) => x.id === c.substitutedFrom)?.name
        : undefined,
      ndc: p.ndc,
      uom: uomLabel,
      uomFactor: factor > 1 ? factor : undefined,
      kitComponents: kitSummary,
  ...
  const lineDiscounts = round2(
    lines.reduce((s, l) => s + lineDiscountOf(l), 0),
  );
  /* bulk tiers apply per non-Rx line */
  const bulkSavings = round2(
    lines.reduce(
      (s, l) =>
        s +
        (l.rx
          ? 0
          : ((grossTotal(l) - lineDiscountOf(l)) * bulkPct(l.qty)) / 100),
      0,
    ),
  );
  const discountBase = Math.max(0, subtotal - lineDiscounts);
  const discount = round2((discountBase * discountPct) / 100);
  const invoiceAmt = round2(
    Math.max(0, Math.min(invoiceDiscountAmt, discountBase - discount)),
  );
  ...
  const loyaltyDeduct = round2(
    Math.min(
      (state.redeemPoints / Math.max(1, loy.chunkPts)) * loy.chunkValue,
      payable,
    ),
  );
...
function withToast(s: State, kind: Toast["kind"], msg: string): State {
  return {
    ...s,
    toasts: [...s.toasts.slice(-3), { id: toastSeq++, kind, msg }],
  };
}
...
function withAudit(s: State, kind: AuditKind, detail: string): State {
  ...
  return {
    ...s,
    audit: [
      { id: nextId, at: Date.now(), actor: s.user?.name ?? "", kind, detail },
      ...s.audit,
    ].slice(0, 250),
  };
}
...
export function listSnapshots(): Snapshot[] {
  try {
    return JSON.parse(localStorage.getItem(SNAPS_KEY) ?? "[]") as Snapshot[];
  } catch {
    return [];
  }
}
function writeSnapshots(snaps: Snapshot[]) {
  try {
    localStorage.setItem(SNAPS_KEY, JSON.stringify(snaps.slice(0, 8)));
  } catch {
    /* full — drop oldest and retry once */
    try {
      localStorage.setItem(SNAPS_KEY, JSON.stringify(snaps.slice(0, 4)));
    } catch {
      /* give up */
    }
  }
...
function listBackups(): BackupRecord[] {
  try {
    return JSON.parse(
      localStorage.getItem(BACKUPS_KEY) ?? "[]",
    ) as BackupRecord[];
  } catch {
    return [];
  }
...
function writeBackups(records: BackupRecord[]) {
  try {
    localStorage.setItem(
      BACKUPS_KEY,
      JSON.stringify(records.slice(0, BACKUP_KEEP)),
    );
  } catch {
    /* storage full — drop oldest and retry */
    try {
      localStorage.setItem(BACKUPS_KEY, JSON.stringify(records.slice(0, 1)));
    } catch {
      /* give up */
    }
  }
  ...
  switch (a.type) {
    case "LOGIN": {
      ...
      if (a.pin !== undefined && s.pinHash !== hashPin(a.pin)) {
        const fails =
          (lock && lock.until <= Date.now() ? 0 : (lock?.fails ?? 0)) + 1;
        const lockouts = {
          ...state.lockouts,
          [s.id]: {
            fails,
            until: fails >= LOCK_AFTER ? Date.now() + LOCK_MS : 0,
          },
        };
      ...
      return withAudit(
        withToast(
          { ...state, user: s, backendAuthenticated: false, lockouts },
          "success",
          i18n.t("toast.signedIn", { name: s.name, role: ROLE_LABEL[s.role] }),
        ),
        "system",
        `${s.name} signed in · role ${s.role} · ${state.settings.terminalId}`,
      );
    }
    ...
    case "LOGOUT":
      return withAudit(
        withToast(
          ...
          "info",
          a.auto
            ? "Terminal locked after inactivity"
            : `${state.user?.name ?? "User"} signed out — terminal locked`,
        ),
        "system",
        `${state.user?.name ?? "User"} ${a.auto ? "auto-locked (idle)" : "signed out"}`,
      );
    ...
    case "ADD_STAFF": {
      ...
      return withAudit(
        withToast(
          { ...state, staff: [s, ...state.staff] },
          "success",
          i18n.t("toast.staffAdded", {
            name: s.name,
            role: ROLE_LABEL[a.role],
          }),
        ),
        "system",
        `Staff created — ${s.name} (${id}, ${a.role})`,
      );
    }
    ...
    case "UPDATE_STAFF": {
      ...
      const staff = state.staff.map((x) =>
        x.id === a.id ? { ...x, ...a.patch } : x,
      );
      const user =
        state.user?.id === a.id ? { ...state.user, ...a.patch } : state.user;
      const what =
        a.patch.active === false
          ? "deactivated"
          : a.patch.active === true
            ? "reactivated"
            : "updated";
      return withAudit(
        ...
        "system",
        `Staff ${what} — ${prev.name}${a.patch.role && a.patch.role !== prev.role ? ` → ${a.patch.role}` : ""}`,
      );
    }

    case "SET_STAFF_PIN": {
      const staff = state.staff.map((x) =>
        x.id === a.id ? { ...x, pinHash: hashPin(a.pin) } : x,
      );
      const lockouts = { ...state.lockouts };
      delete lockouts[a.id];
      return withAudit(
        withToast(
          { ...state, staff, lockouts },
          "success",
          i18n.t("toast.pinReset"),
        ),
        "system",
        `PIN reset for ${state.staff.find((x) => x.id === a.id)?.name ?? a.id}`,
      );
    }
    ...
    case "ADD_PRESCRIBER": {
      ...
      return withAudit(
        withToast(
          { ...state, prescribers: [...state.prescribers, pr] },
          "success",
          i18n.t("toast.prescriberAdded", { name: pr.name }),
        ),
        "rx",
        `Prescriber added — ${pr.name} · NPI ${pr.npi}`,
      );
    }
    ...
    case "PRESCRIBER_SAVE": {
      if (
        !can(state.user?.role, "manage_settings") &&
        state.user?.role !== "pharmacist"
      ) {
        return withToast(
          state,
          "error",
          "Pharmacist or admin required to manage prescribers",
        );
      }
      const exists = state.prescribers.findIndex(
        (p) => p.id === a.prescriber.id,
      );
      const prescriber: Prescriber =
        !a.prescriber.id && exists < 0
          ? {
              ...a.prescriber,
              id: `DR-${String(state.prescribers.length + 1).padStart(2, "0")}`,
            }
          : a.prescriber;
      const prescribers =
        exists >= 0
          ? state.prescribers.map((p) =>
              p.id === a.prescriber.id ? prescriber : p,
            )
      ...
      return withToast(
        withAudit(
          ...
          "rx",
          exists >= 0
            ? `Prescriber ${prescriber.name} updated`
            : `Prescriber ${prescriber.name} created`,
        ),
        "success",
        exists >= 0
          ? i18n.t("prescribers.updated", { name: prescriber.name })
          : i18n.t("prescribers.created", { name: prescriber.name }),
      );
    }
    ...
    case "PRESCRIBER_DELETE": {
      ...
      if (!pr) return state;
      const referenced = state.prescriptions.some(
        (rx) => rx.prescriberId === a.id,
      );
      if (referenced)
        return withToast(state, "error", i18n.t("prescribers.deleteBlocked"));
      const prescribers = state.prescribers.filter((p) => p.id !== a.id);
      return withToast(
        withAudit(
          { ...state, prescribers },
          "rx",
          `Prescriber ${pr.name} deleted`,
        ),
        "success",
        i18n.t("prescribers.deleted", { name: pr.name }),
      );
    }
    ...
    case "FLAG_RECALL": {
        ...
        x.id === p.id
          ? {
              ...
              batches: x.batches.map((b) =>
                b.batch === a.batch
                  ? { ...b, recalled: a.flagged || undefined }
                  : b,
              ),
            }
          : x,
      ...
      return withToast(
        next,
        a.flagged ? "warn" : "success",
        a.flagged
          ? `Lot ${a.batch} flagged for recall — trace affected patients`
          : `Recall flag cleared on ${a.batch}`,
      );
    }

    case "TOGGLE_RESTRICTED": {
      const products = state.products.map((p) =>
        p.id === a.productId ? { ...p, restricted: a.restricted } : p,
      );
      return withAudit(
        { ...state, products },
        "system",
        a.restricted
          ? `Restricted OTC flag set on ${state.products.find((p) => p.id === a.productId)?.name ?? a.productId} (limit ${a.restricted.limitPerSale}/sale)`
    ...
    case "SAVE_UOMS": {
      ...
      if (!p) return state;
      const products = state.products.map((x) =>
        x.id === p.id ? { ...x, uoms: a.uoms } : x,
      );
      const label = a.uoms.length
        ? a.uoms
            .map((u) => `${u.label} (×${u.factor} @ ${money(u.price)})`)
            .join(" · ")
        : "none — base unit only";
      return withToast(
        withAudit(
          { ...state, products },
          "stock",
          `UOM packs updated — ${p.name}: ${label}`,
        ),
        "success",
        `${p.name} — ${a.uoms.length} unit${a.uoms.length === 1 ? "" : "s"} of measure saved`,
      );
    }
    ...
    case "RTV": {
      ...
      const products = state.products.map((x) =>
        x.id === p.id
          ? { ...x, batches: deductFromLot(x.batches, a.batch, a.qty) }
          : x,
      );
      ...
      if (sup) {
        const open = state.apInvoices.find(
          (inv) => inv.supplierId === sup.id && invoiceBalance(inv) > 0,
        );
        if (open) {
              ...
              ? {
                  ...
                  credits: [
                    ...inv.credits,
                    {
                      at: Date.now(),
                      amount: value,
                      note: `RTV ${a.qty}× ${p.name} (lot ${a.batch}) — ${reason}`,
                    },
                  ],
                }
              : inv,
      ...
      return withToast(
        next,
        "success",
        `RTV booked — ${money(value)} credit against ${p.supplier}`,
      );
    }
    ...
    case "WRITE_OFF": {
      if (!can(state.user?.role, "apply_count")) {
        return withToast(
          state,
          "error",
          "Write-offs require a manager or admin approval",
        );
      }
      const p = state.products.find((x) => x.id === a.productId);
      if (!p) return state;
      ...
      const products = state.products.map((x) =>
        x.id === p.id
          ? { ...x, batches: x.batches.filter((b) => b.batch !== a.batch) }
          : x,
      );
      ...
      return withToast(
        next,
        "warn",
        `Wrote off ${lot.qty} × ${p.name} — lot ${a.batch} removed from shelf`,
      );
    }
    ...
    case "COLD_CHAIN_LOG": {
      ...
      const entry: ColdChainLog = {
        id: `CCL-${Date.now().toString(36)}`,
        productId: p.id,
        tempC: round2(a.tempC),
        inRange: tempInRange(a.tempC),
        staff: state.user?.name ?? "Staff",
        note: a.note?.trim() || undefined,
        at: Date.now(),
      };
      const next = withAudit(
        { ...state, coldChainLog: [entry, ...state.coldChainLog] },
        "stock",
        `Cold-chain temp ${entry.tempC}°C ${entry.inRange ? "✓" : "⚠ OUT OF RANGE"} — ${p.name}${entry.note ? ` · ${entry.note}` : ""}`,
      );
      return withToast(
        next,
        entry.inRange ? "success" : "warn",
    ...
    case "UPDATE_SETTINGS": {
      const settings = {
        ...state.settings,
        ...a.patch,
        loyalty: { ...state.settings.loyalty, ...(a.patch.loyalty ?? {}) },
      };
      return withAudit(
        { ...state, settings },
        "system",
    ...
    case "SNAPSHOT_SAVE": {
      ...
      const data = {
        ...
        transactions: state.transactions.slice(0, 400),
      ...
      return withToast(
        { ...state, snapshotVersion: state.snapshotVersion + 1 },
        "success",
        i18n.t("toast.snapshotSaved", { label: a.label }),
      );
    }
    ...
    case "SNAPSHOT_RESTORE": {
      ...
      if (!snap)
        return withToast(state, "error", i18n.t("toast.snapshotNotFound"));
      const d = snap.data as Partial<State>;
      return withToast(
        withAudit(
          {
            ...state,
            products: d.products ?? state.products,
            transactions: d.transactions ?? state.transactions,
            prescriptions: d.prescriptions ?? state.prescriptions,
            customers: d.customers ?? state.customers,
            transfers: d.transfers ?? state.transfers,
            staff: d.staff ?? state.staff,
            settings: { ...makeSettings(), ...(d.settings ?? {}) },
          ...
          "system",
          `Snapshot restored — ${snap.meta.label}`,
        ),
        "success",
    ...
    case "GO":
      return {
        ...state,
        view: a.view,
        invPreset: a.invPreset ?? state.invPreset,
        payOpen: false,
      };
    ...
    case "ADD_CART": {
      ...
      if (avail <= 0)
        return withToast(
          state,
          "error",
          i18n.t("toast.outOfStock", { name: p.name }),
        );
      ...
      const same = (c: { productId: string; uom?: string }) =>
        c.productId === p.id && (c.uom ?? "") === (a.uom ?? "");
      ...
      if (line) {
        if (line.qty >= maxUom)
          return withToast(
            state,
            "warn",
            i18n.t("toast.lowUnits", { avail, name: p.name }),
          );
        return {
          ...state,
          flashId: p.id,
      ...
      return {
        ...
        cart: [
          ...state.cart,
          {
            ...
            uom: a.uom,
            daw: a.daw,
            substitutedFrom: a.substitutedFrom,
          },
        ],
      };
    }
    ...
    case "NOTIFY_RX": {
      ...
      const prescriptions = state.prescriptions.map((x) =>
        x.id === a.id ? { ...x, notifiedAt: Date.now() } : x,
      );
      return withToast(
        withAudit(
          { ...state, prescriptions },
          "rx",
          `Pickup notification sent — ${rx.id} · ${rx.patient}${rx.phone ? ` · ${rx.phone}` : ""}`,
        ),
        "success",
        `"Ready for pickup" sent to ${rx.patient}${rx.phone ? ` · ${rx.phone}` : ""}`,
      );
    }
    ...
    case "PA_SUBMIT": {
        ...
        x.id === a.id
          ? {
              ...
              pa: {
                ...
                note: `Submitted to ${x.insurance?.plan ?? "payer"} — awaiting clinical review`,
              },
            }
          : x,
      );
      return withToast(
        withAudit(
          { ...state, prescriptions },
          "rx",
          `Prior auth requested — ${rx.id} · ${rx.patient} · ${rx.insurance?.plan ?? "payer"}`,
        ),
        "success",
        `PA submitted for ${rx.id} — tracking with ${rx.insurance?.plan ?? "the payer"}`,
      );
    }
    ...
    case "PA_CHECK": {
      ...
      const pa = ok
        ? {
            ...
            requestedAt: rx.pa.requestedAt,
            decidedAt: Date.now(),
            note: "Approved — clinical criteria met",
          }
        : {
            status: "rejected" as const,
            requestedAt: rx.pa.requestedAt,
            decidedAt: Date.now(),
            note: "Clinical criteria not met — resubmit with chart notes",
          };
      const prescriptions = state.prescriptions.map((x) =>
        x.id === a.id ? { ...x, pa } : x,
      );
      ...
      return ok
        ...
        : withToast(
            next,
            "error",
            i18n.t("toast.paRejected", { id: rx.id, note: pa.note }),
          );
    }
    ...
    case "PA_RESUBMIT": {
        ...
        x.id === a.id
          ? {
              ...
              pa: {
                status: "pending" as const,
                requestedAt: Date.now(),
                note: "Resubmitted with supporting chart notes",
              },
            }
          : x,
      );
      return withToast(
        withAudit(
          { ...state, prescriptions },
          "rx",
          `Prior auth resubmitted — ${rx.id} · ${rx.patient}`,
        ),
        "info",
        `${rx.id} PA resubmitted — back in the payer queue`,
      );
    }
    ...
    case "CREATE_BACKORDER": {
      const p = state.products.find((x) => x.id === a.productId);
      if (!p) return state;
      const dupe = state.backorders.find(
        (b) =>
      ...
      if (dupe)
        return withToast(
          state,
          "warn",
          i18n.t("toast.dupeBackorder", { patient: a.patient, name: p.name }),
        );
      ...
      return withToast(
        withAudit(
          { ...state, backorders: [bo, ...state.backorders] },
          "stock",
          `Back-order ${id} — ${a.qty} × ${p.name} for ${a.patient} · ETA 3d · ${p.supplier}`,
        ),
        "success",
        `Back-order ${id} placed for ${a.patient} — we'll notify when it lands`,
      );
    }
    ...
    case "BACKORDER_STATUS": {
      ...
      const now = Date.now();
      const backorders = state.backorders.map((x) =>
        x.id === a.id
          ? {
              ...x,
              status: a.to,
              arrivedAt:
                a.to === "arrived" ? (x.arrivedAt ?? now) : x.arrivedAt,
              notifiedAt:
                a.to === "notified" ? (x.notifiedAt ?? now) : x.notifiedAt,
            }
          : x,
      );
      const verb =
      ...
      return withToast(
        withAudit(
          { ...state, backorders },
          "stock",
          `Back-order ${bo.id} ${verb} — ${bo.qty} × ${p?.name ?? bo.productId} · ${bo.patient}`,
        ),
        a.to === "cancelled" ? "info" : "success",
        `${bo.id} ${verb}`,
      );
    }
    ...
    case "TRANSFER_RX_OUT": {
      ...
      const rec: RxTransfer = {
        ...
        patient: rx.patient,
        drug: `${p?.name ?? rx.productId} × ${rx.qty}`,
        qty: rx.qty,
        otherPharmacy: a.otherPharmacy,
        otherPhone: a.otherPhone,
        prescriber:
          state.prescribers.find((x) => x.id === rx.prescriberId)?.name ??
          rx.prescriberId,
        refillsRemaining: a.refillsRemaining,
        pharmacist: state.user?.name ?? "Pharmacist on duty",
        at: Date.now(),
        note: a.note?.trim() || undefined,
      };
      const prescriptions = state.prescriptions.map((x) =>
        x.id === rx.id
          ? { ...x, transferredOut: { at: Date.now(), to: a.otherPharmacy } }
          : x,
      );
      return withToast(
        withAudit(
          ...
          "rx",
          `Rx transfer OUT ${transferNo} — ${rx.id} · ${rx.patient} → ${a.otherPharmacy} (${a.refillsRemaining} refills)`,
        ),
        "success",
        `${transferNo} sent to ${a.otherPharmacy} — fill authority released`,
      );
    }
        ...
        withAudit(
          {
            ...
            rxTransfers: [rec, ...state.rxTransfers],
          },
          "rx",
          `Rx transfer IN ${transferNo} — ${a.patient} · ${p.name} from ${a.otherPharmacy} → ${rxId}`,
        ),
        "success",
        `${rxId} created from ${a.otherPharmacy}'s transfer — queued for review`,
      );
    }
    ...
    case "NEW_PRESCRIPTION": {
      ...
      const prescriber = state.prescribers.find(
        (x) => x.id === intake.prescriberId,
      );
      if (!p || !prescriber) return state;
      const rxId = `RX-${2490 + state.prescriptions.length}`;
      const rx: Prescription = {
        id: rxId,
        patient: intake.patient.trim(),
        age: intake.age,
        ...
        refillsRemaining: intake.refillsAuthorized,
        rxExpiry: intake.rxExpiry,
        note: intake.note?.trim() || undefined,
        insurance: intake.insurancePlan?.trim()
          ? {
              plan: intake.insurancePlan.trim(),
              memberId: intake.memberId?.trim() || "—",
              status: "pending",
            }
          : undefined,
      };
      return withToast(
        withAudit(
          ...
          "rx",
          `New Rx intake ${rxId} — ${rx.patient} · ${p.name} × ${rx.qty} · ${prescriber.name}${rx.insurance ? ` · ${rx.insurance.plan}` : ""}`,
        ),
        "success",
        `${rxId} dropped off — queued for pharmacist review`,
      );
    }
    ...
    case "SCAN_ATTACH": {
      ...
      if (!rx) return state;
      const prescriptions = state.prescriptions.map((x) =>
        x.id === a.id ? { ...x, scan: a.dataUrl, scanAt: Date.now() } : x,
      );
      return withToast(
        withAudit(
          { ...state, prescriptions },
          "rx",
          `Hard-copy scan attached to ${rx.id} — ${rx.patient}`,
        ),
        "success",
        `Scan attached to ${rx.id} — stored with the prescription`,
      );
    }
    ...
    case "CUSTOMER_ALLERGIES": {
      ...
      const customers = state.customers.map((x) =>
        x.id === a.id
          ? { ...x, allergies: a.allergies.length ? a.allergies : undefined }
          : x,
      );
      return withToast(
        withAudit(
          { ...state, customers },
          "rx",
          `Allergy profile updated — ${c.name}: ${a.allergies.length ? a.allergies.join(", ") : "none on file"}`,
        ),
        "success",
        a.allergies.length
          ? `${c.name} — ${a.allergies.length} allergen${a.allergies.length === 1 ? "" : "s"} on file`
          : `${c.name} — allergies cleared`,
      );
    }
    ...
    case "COMPOUND": {
      ...
      for (const ing of a.ingredients) {
        ...
        if (!src || stockOf(src) < ing.qty) {
          return withToast(
            state,
            "error",
            `Not enough ${src?.name ?? "stock"} on hand to compound`,
          );
        }
      }
      let cost = 0;
      ...
      const compound: Product = {
        id,
        sku: `CMP-${id.slice(-5).toUpperCase()}`,
        barcode: `892${String(Math.floor(Math.random() * 1e10)).padStart(10, "0")}`,
        name: a.name.trim(),
        generic: "Compounded preparation",
        brand: "In-house compound",
        category: "compound",
        form: `Compound · ${a.ingredients.length} ingredient${a.ingredients.length === 1 ? "" : "s"}`,
        price: round2(a.price),
        cost,
        reorderLevel: 2,
        rx: true,
        supplier: "Compounded in-house",
        compound: true,
        batches: [
          {
            batch: newBatchCode(),
      ...
      const ingLabel = a.ingredients
        .map(
          (i) =>
            `${i.qty}× ${state.products.find((x) => x.id === i.productId)?.name ?? i.productId}`,
        )
        .join(" + ");
      return withToast(
        withAudit(
          ...
          "stock",
          `Compound batch made — ${compound.name} (${ingLabel}) · cost ${money(cost)} · exp ${compound.batches[0].expiry}`,
        ),
        "success",
        `${compound.name} compounded — 1 unit in stock @ ${money(compound.price)}`,
      );
    }
    ...
    case "SET_QTY": {
      ...
      const same = (c: { productId: string; uom?: string }) =>
        c.productId === a.productId && (c.uom ?? "") === (a.uom ?? "");
      const qty = Math.min(Math.max(0, a.qty), maxUom);
      if (a.qty > maxUom)
        return withToast(state, "warn", `Only ${avail} base units in stock`);
      if (qty === 0)
        return { ...state, cart: state.cart.filter((c) => !same(c)) };
      return {
        ...state,
        cart: state.cart.map((c) => (same(c) ? { ...c, qty } : c)),
      };
    }
    ...
    case "SET_LINE_UOM": {
      ...
      if (!p) return state;
      const factor = uomFactor(state, p.id, a.uom);
      const maxUom = Math.max(
        1,
        Math.floor(stockOf(p, state.products) / factor),
      );
      const cart = state.cart.map((c) =>
        c.productId === a.productId
          ? {
              ...c,
              uom: a.uom || undefined,
              qty: Math.min(Math.max(1, c.qty), maxUom),
            }
          : c,
      );
      return { ...state, cart };
    ...
    case "HOLD_SALE": {
      ...
      const h: HeldSale = {
        id: `H-${heldSeq++}`,
        label: a.label || `Hold ${state.held.length + 1}`,
        at: Date.now(),
        expiresAt: a.expiresAt,
        items: state.cart,
      };
      return withToast(
        { ...state, held: [...state.held, h], cart: [] },
        "info",
        `Sale parked as “${h.label}”`,
      );
    }
    ...
    case "EXPIRE_HELDS": {
      ...
      return withAudit(
        { ...state, held: pruned },
        "shift",
        `${dropped} layaway/expired hold(s) auto-removed`,
      );
    }

    case "ISSUE_STORE_CREDIT": {
      if (!can(state.user?.role, "refund"))
        return withToast(
          state,
          "error",
          "Manager approval required to issue store credit",
        );
      return withToast(
        withAudit(
          { ...state, storeCredits: [...state.storeCredits, a.credit] },
          "money",
          `Store credit ${a.credit.id} issued — ${money(a.credit.balance)}${a.credit.code ? ` (code ${a.credit.code})` : ""}`,
        ),
        "success",
        `Store credit issued — ${money(a.credit.balance)}`,
      );
    }
    ...
    case "REDEEM_STORE_CREDIT": {
      ...
      return {
        ...state,
        storeCredits: applyStoreCredit(state.storeCredits, a.id, a.amount),
      };
    }

    case "SAVE_COUPON": {
      if (!can(state.user?.role, "manage_settings"))
        return withToast(state, "error", "Admin required to manage coupons");
      const exists = state.coupons.findIndex((c) => c.id === a.coupon.id);
      const coupons =
        exists >= 0
          ? state.coupons.map((c) => (c.id === a.coupon.id ? a.coupon : c))
          : [...state.coupons, a.coupon];
      return withToast(
        withAudit(
          { ...state, coupons },
          "settings",
          exists >= 0
            ? `Coupon ${a.coupon.code} updated`
            : `Coupon ${a.coupon.code} created`,
        ),
        "success",
        exists >= 0
          ? i18n.t("toast.couponUpdated", { code: a.coupon.code })
          : i18n.t("toast.couponCreated", { code: a.coupon.code }),
      );
    }
    ...
    case "DELETE_COUPON": {
      ...
      return withToast(
        withAudit({ ...state, coupons }, "settings", `Coupon ${a.id} deleted`),
        "success",
        i18n.t("toast.couponDeleted"),
      );
    }
    ...
    case "SAVE_CATEGORY": {
      if (!can(state.user?.role, "manage_settings"))
        return withToast(state, "error", "Admin required to manage categories");
      const exists = state.categories.findIndex((c) => c.id === a.category.id);
      const categories =
        exists >= 0
          ? state.categories.map((c) =>
              c.id === a.category.id ? a.category : c,
            )
      ...
      return withToast(
        withAudit(
          ...
          "settings",
          exists >= 0
            ? `Category ${a.category.label} updated`
            : `Category ${a.category.label} created`,
        ),
        "success",
        `Category saved — ${a.category.label}`,
      );
    }
    ...
    case "DELETE_CATEGORY": {
      ...
      if (inUse)
        return withToast(
          state,
          "error",
          "Category still assigned to products — archive it instead",
        );
    ...
    case "SUPPLIER_SAVE": {
      if (!can(state.user?.role, "manage_settings"))
        return withToast(state, "error", "Admin required to manage suppliers");
      ...
      return withToast(
        ...
        "success",
        exists >= 0
          ? i18n.t("suppliers.updated", { name: supplier.name })
          : i18n.t("suppliers.created", { name: supplier.name }),
      );
    }
    ...
    case "SUPPLIER_DELETE": {
      ...
      if (!sup) return state;
      const usedByProducts = state.products.some(
        (p) => p.supplier === sup.name,
      );
      const usedByOrders = state.purchaseOrders.some(
        (po) => po.supplierId === a.id,
      );
      if (usedByProducts || usedByOrders)
        return withToast(state, "error", i18n.t("suppliers.deleteBlocked"));
      const suppliers = state.suppliers.filter((s) => s.id !== a.id);
      return withToast(
        withAudit(
          { ...state, suppliers },
          "settings",
          `Supplier ${sup.name} deleted`,
        ),
        "success",
        i18n.t("suppliers.deleted", { name: sup.name }),
      );
    }
    ...
    case "RECALL_HELD": {
      ...
      if (state.cart.length > 0) {
        ...
        return withToast(
          ...
          "info",
          `Recalled “${h.label}” — current cart auto-parked`,
        );
      }
      return withToast(
        { ...state, held: heldNow, cart: h.items, view: "register" },
        "success",
        `Recalled “${h.label}”`,
      );
    }
    ...
    case "COMPLETE_SALE": {
      ...
      const controlledLines = t.lines.filter(
        (l) => state.products.find((p) => p.id === l.productId)?.controlled,
      );
      if (controlledLines.length > 0) {
        if (!customer)
          return withToast(
            state,
            "error",
            "Controlled substance in cart — attach a customer (photo ID required)",
          );
        if (!a.idChecked)
          return withToast(
            state,
            "error",
            "Confirm the ID check before completing a controlled sale",
          );
      }
      ...
      for (const l of t.lines) {
        ...
        if (p.kit && p.kit.length > 0) {
          for (const comp of p.kit)
            demand.set(
              comp.productId,
              (demand.get(comp.productId) ?? 0) + l.qty * comp.qty,
            );
        } else {
          demand.set(
            p.id,
            (demand.get(p.id) ?? 0) + l.qty * (l.uomFactor ?? 1),
          );
        }
      }
      ...
      for (const [pid, need] of demand) {
        ...
        if (onHand < need)
          return withToast(
            state,
            "error",
            `${p.name} short on stock — only ${onHand} base units left`,
          );
      }
      ...
      const products = state.products.map((p) => {
        ...
        const fifoCost =
          allocQty > 0
            ? round2(
                res.alloc.reduce((s, x) => s + x.qty * (x.cost ?? p.cost), 0) /
                  allocQty,
              )
            : p.cost;
      ...
      const pointsEarned = Math.floor(
        t.total * state.settings.loyalty.ptsPerUnit,
      );
        ...
        ? state.customers.map((c) =>
            c.id === customer.id
              ? {
                  ...
                  points: Math.max(
                    0,
                    c.points + pointsEarned - state.redeemPoints,
                  ),
                }
              : c,
          )
      ...
      const tx: Transaction = {
        ...
        total: t.total,
        invoiceDiscountAmt:
          (a.invoiceDiscountAmt ?? 0) > 0 ? t.invoiceAmt : undefined,
        method: primary.method,
        cashier: state.user?.name ?? "Staff",
        ...
        change: singleCash
          ? round2((a.tendered ?? primary.amount) - t.total)
          : undefined,
      ...
      const creditDeductions = a.payments.filter(
        (p) => p.method === "store_credit" && p.ref,
      );
      ...
      if (payLaterLeg && customer) {
        const dueStr = payLaterLeg.dueDate
          ? new Date(payLaterLeg.dueDate).toLocaleDateString()
          : "TBD";
        next = withAudit(
          next,
          "sale",
          `${tx.id} · pay_later $${payLaterLeg.amount.toFixed(2)} due ${dueStr} · ${customer.name}`,
        );
      }
      const ptsLabel = customer
        ? ` · +${pointsEarned}${state.redeemPoints ? ` / −${state.redeemPoints} pts` : ""} pts`
        : "";
      next = withAudit(
        next,
        "sale",
      ...
      if (next.currentShift) {
        const updated = recordShiftTransaction(
          ...
          tx,
          "sale",
          tenderTypeOf(primary.method),
        );
        next = {
          ...next,
          shifts: next.shifts.map((s) => (s.id === updated.id ? updated : s)),
      ...
      if (controlledLines.length > 0 && customer) {
        next = withAudit(
          next,
          "rx",
          `⚠ Controlled sale ${tx.id} — ${controlledLines.map((l) => `${l.name} ×${l.qty}`).join(", ")} · ${customer.name} · ID verified ✓`,
        );
      }
      /* charge-on-pickup: any cart line stamped with an rxId dispenses that Rx on payment (W1.4) */
      const pickupRxIds = [
        ...new Set(
          state.cart.map((c) => c.rxId).filter((id): id is string => !!id),
        ),
      ].filter((id) =>
        next.prescriptions.some((x) => x.id === id && x.status !== "dispensed"),
      );
      if (pickupRxIds.length > 0) {
        next = {
          ...next,
          prescriptions: next.prescriptions.map((x) =>
            pickupRxIds.includes(x.id)
              ? {
                  ...x,
                  status: "dispensed" as RxStatus,
                  dispensedAt: x.dispensedAt ?? tx.at,
                  refillsRemaining:
                    x.refillsRemaining !== undefined
                      ? Math.max(0, x.refillsRemaining - 1)
                      : x.refillsRemaining,
                }
              : x,
          ),
        ...
        for (const rxId of pickupRxIds) {
          ...
          next = withAudit(
            next,
            "rx",
            `${rxId} dispensed on pickup via ${tx.id} — ${rx.patient} · ${rx.qty} × ${state.products.find((p) => p.id === rx.productId)?.name ?? rx.productId}`,
          );
        }
      }
      ...
      if (a.restricted && a.restricted.purchaser.trim()) {
        ...
        const entries: RestrictedLogEntry[] = t.lines
          ...
          .map(({ p, l }) => ({
            ...
            productId: p.id,
            qty: l.qty,
            purchaser: a.restricted!.purchaser.trim(),
            idType: a.restricted!.idType,
            idLast4: a.restricted!.idLast4,
            cashier: state.user?.name ?? "Staff",
          }));
        ...
        next = withAudit(
          next,
          "sale",
          `⚠ BTC log ${tx.id} — ${entries.map((e) => `${e.qty}× ${state.products.find((p) => p.id === e.productId)?.name ?? e.productId}`).join(", ")} · ${a.restricted!.purchaser} (${a.restricted!.idType} ····${a.restricted!.idLast4})`,
        );
      }
      return withToast(
        next,
        "success",
        `Payment captured — ${tx.id} · $${t.total.toFixed(2)} · ${tenderLabel}${ptsLabel}`,
      );
    }
    ...
    case "SETTLE_PAY_LATER": {
      ...
      if (!tx.payments || !tx.payments[a.legIndex])
        return withToast(state, "error", "Payment leg not found");
      const leg = tx.payments[a.legIndex];
      if (leg.method !== "pay_later")
        return withToast(state, "error", "Not a pay_later leg");
      if (leg.settledAt) {
        const settledDate = new Date(leg.settledAt).toLocaleDateString();
        return withToast(
          state,
          "error",
          `Cannot settle — already settled on ${settledDate}`,
        );
      }
      const now = Date.now();
      ...
      return withToast(
        next,
        "success",
        `Pay later settled — $${leg.amount.toFixed(2)} via ${a.method}`,
      );
    }
          ...
          : {
              ...
              batches:
                newQty === 0
                  ...
                  : x.batches.map((bb) =>
                      bb.batch === a.batch ? { ...bb, qty: newQty } : bb,
                    ),
            },
      );
      const next = withAudit(
        { ...state, products },
        "stock",
        `${a.reason} — ${p.name} · ${a.batch} ${delta >= 0 ? "+" : ""}${delta} → ${newQty}`,
      );
      return withToast(
        next,
    ...
    case "RESTOCK": {
      ...
      const lot: Batch = {
        ...
        expiry: a.expiry,
        qty: a.amount,
        cost: a.cost !== undefined ? round2(a.cost) : p.cost,
      };
      const products = state.products.map((x) =>
        x.id === a.productId ? { ...x, batches: [...x.batches, lot] } : x,
      );
      /* receiving stock flips any matching open back-orders to "arrived" (§3) */
      const matched = state.backorders.filter(
        (b) => b.productId === a.productId && b.status === "ordered",
      );
      const backorders =
        matched.length > 0
          ? state.backorders.map((b) =>
              b.productId === a.productId && b.status === "ordered"
                ? { ...b, status: "arrived" as const, arrivedAt: Date.now() }
                : b,
            )
          : state.backorders;
      const suffix =
        matched.length > 0
          ? ` — ${matched.length} back-order${matched.length === 1 ? "" : "s"} now in`
          : "";
      const costLabel = ` @ ${money(lot.cost!)}/unit`;
      const next = withAudit(
        { ...state, products, backorders },
        "stock",
        `Received +${a.amount} × ${p.name} → lot ${a.batch} (exp ${a.expiry})${costLabel}${suffix}`,
      );
      return withToast(
        next,
        "success",
        `Received +${a.amount} × ${p.name} → lot ${a.batch}${costLabel}${suffix}`,
      );
    }
    ...
    case "SET_NOTE": {
      ...
      return {
        ...
        cart: state.cart.map((c) =>
          c.productId === a.productId ? { ...c, note: note || undefined } : c,
        ),
      };
    }
    ...
    case "SET_PRICE": {
      ...
      const cart = state.cart.map((c) =>
        c.productId === a.productId
          ? {
              ...c,
              priceOverride: a.price === null ? undefined : round2(a.price),
            }
          : c,
      );
      return a.price === null
        ? withToast(
            { ...state, cart },
            "info",
            `${p.name} back to list price ${money(p.price)}`,
          )
        : withToast(
            withAudit(
              { ...state, cart },
              "money",
              `Price override — ${p.name} → ${money(round2(a.price))} (list ${money(p.price)})`,
            ),
            "success",
            `${p.name} overridden to ${money(round2(a.price))} (list ${money(p.price)})`,
          );
    }
    ...
    case "SET_LINE_DISCOUNT": {
      ...
      if (!a.discount || a.discount.value <= 0) {
        const cart = state.cart.map((c) =>
          same(c) ? { ...c, lineDiscount: undefined } : c,
        );
        return withToast(
          { ...state, cart },
          "info",
          `${p.name} line discount removed`,
        );
      }
      ...
      const frac =
        a.discount.mode === "pct"
          ? Math.min(100, a.discount.value) / 100
          : Math.min(1, a.discount.value / Math.max(0.01, gross));
      if (
        frac >= LINE_DISCOUNT_PIN_THRESHOLD &&
        !can(state.user?.role, "approve_discount") &&
        !a.approvedBy
      ) {
        return withToast(
          state,
          "error",
          `Discount of ${(frac * 100).toFixed(0)}% needs manager PIN approval`,
        );
      }
      const cart = state.cart.map((c) =>
        same(c) ? { ...c, lineDiscount: a.discount } : c,
      );
      return withToast(
        withAudit(
          { ...state, cart },
    ...
    case "SET_BATCH_PRICE": {
      ...
      const products = state.products.map((x) =>
        x.id !== p.id
          ...
          : {
              ...x,
              batches: x.batches.map((bb) =>
                bb.batch === a.batch
                  ? {
                      ...bb,
                      price: a.price === null ? undefined : round2(a.price),
                    }
                  : bb,
              ),
            },
      ...
      return withToast(
        ...
        "success",
        a.price === null
          ? `${a.batch} back to list price`
          : `Lot ${a.batch} priced at ${money(round2(a.price))} (list ${money(p.price)})`,
      );
    }
    ...
    case "ADD_TRANSFER": {
      ...
      if (a.qty > onHand)
        return withToast(state, "error", `Only ${onHand} × ${p.name} on hand`);
      ...
      const tr: Transfer = {
        ...
        status: "requested",
        createdAt: Date.now(),
        requestedBy: state.user?.name ?? "Staff",
        note: a.note?.trim() || undefined,
      };
      const next = withAudit(
        { ...state, transfers: [tr, ...state.transfers] },
        "stock",
        `Transfer ${id} requested — ${a.qty} × ${p.name} → ${a.toBranch}`,
      );
      return withToast(
        next,
        "success",
        `${id} requested — ${a.qty} × ${p.name} → ${a.toBranch}`,
      );
    }
    ...
    case "TRANSFER_STATUS": {
      ...
      if (a.status === "shipped" && p) {
        products = state.products.map((x) =>
          x.id === p.id
            ? { ...x, batches: allocFEFO(x.batches, tr.qty).batches }
            : x,
        );
      }
      const transfers = state.transfers.map((x) =>
        x.id === tr.id ? { ...x, status: a.status } : x,
      );
      const verb =
        a.status === "approved"
          ? "approved"
          : a.status === "shipped"
            ? "shipped — stock allocated"
            : a.status === "received"
      ...
      const next = withAudit(
        ...
        "stock",
        `Transfer ${tr.id} ${verb} (${tr.qty} × ${p?.name ?? tr.productId})`,
      );
      return withToast(
        next,
        a.status === "rejected" ? "warn" : "success",
        `${tr.id} ${verb}`,
      );
    }
    ...
    case "SET_FIELD": {
      if (a.target === "product") {
        const products = state.products.map((x) => {
          ...
          return {
            ...x,
            fields: a.field.value.trim() ? [...rest, a.field] : rest,
          };
        });
    ...
    case "CLEAR_FIELD": {
      if (a.target === "product") {
        const products = state.products.map((x) =>
          x.id === a.id
            ? { ...x, fields: (x.fields ?? []).filter((f) => f.key !== a.key) }
            : x,
        );
        return { ...state, products };
      }
    ...
    case "SET_ONLINE":
      ...
      return withToast(
        { ...state, online: a.online },
        a.online ? "success" : "warn",
        a.online
          ? "Back online — local changes will sync"
          : "Offline — sales keep working, saved locally",
      );

    case "SET_SALE_CUSTOMER":
      return { ...state, saleCustomerId: a.id, redeemPoints: 0 };

    case "SET_OUTBOX_COUNT":
      if (a.count === state.outboxCount) return state;
      return { ...state, outboxCount: a.count };

    case "ADD_CONFLICTS":
      return { ...state, conflicts: [...state.conflicts, ...a.conflicts] };

    case "KEEP_LOCAL_CONFLICT":
      return {
        ...state,
        conflicts: state.conflicts.filter(
          (c) => !(c.table === a.table && c.key === a.key),
        ),
      };
    ...
    case "ADD_CUSTOMER": {
      ...
      if (existing) {
        return withToast(
          ...
          "info",
          `${existing.name} already on file — attached to this sale`,
        );
      }
      ...
      return withToast(
        withAudit(
          ...
          "system",
          `Customer created — ${name} (${id})`,
        ),
        "success",
        `${name} added to the book — earning points from this sale`,
      );
    }
    ...
    case "VERIFY_RX": {
      ...
      if (state.user?.role === "cashier") {
        return withToast(
          state,
          "error",
          "Eligibility checks require a pharmacist or manager sign-in",
        );
      }
      ...
      return ok
        ? withToast(
            next,
            "success",
            `${rx.id} · ${rx.insurance.plan} claim verified ✓`,
          )
        : withToast(
            next,
            "error",
            `${rx.id} claim rejected by ${rx.insurance.plan} — check member eligibility`,
          );
    }
    ...
    case "COUNT_APPLY": {
      ...
      const products = state.products.map((p) => {
        ...
        if (!first) {
          return {
            ...
            batches: [
              {
                batch: newBatchCode(),
                expiry: new Date(Date.now() + 365 * 86_400_000)
                  .toISOString()
                  .slice(0, 10),
                qty: e.counted,
              },
        ...
        return {
          ...p,
          batches: newQty > 0 ? [{ ...first, qty: newQty }, ...rest] : rest,
        };
      });
      if (skus === 0)
        return withToast(state, "info", "Count sheet clean — no variances");
      const next = withAudit(
        { ...state, products },
        "stock",
      ...
      return withToast(
        next,
        "success",
        `Count applied — ${skus} variance${skus === 1 ? "" : "s"}, ${units >= 0 ? "+" : ""}${units} units`,
      );
    }
      ...
      return withToast(
        {
          ...
          prescriptions: state.prescriptions.map((x) =>
            x.id === a.id ? { ...x, remindedAt: Date.now() } : x,
          ),
        },
        "info",
        `Refill reminder sent to ${rx.patient} (${rx.id})`,
      );
    }
    ...
    case "NEW_REFILL": {
      ...
      if (!rx) return state;
      const nextNum =
        2482 + state.prescriptions.filter((x) => x.id.startsWith("RX-")).length;
      const copy: Prescription = {
        ...
        refillsRemaining: rx.refillsAuthorized,
        rxExpiry: rx.rxExpiry,
        daysSupply: rx.daysSupply,
        note: `Refill of ${rx.id} · ${rx.patient}`,
      };
      return withToast(
        {
          ...state,
          prescriptions: [copy, ...state.prescriptions],
          view: "prescriptions",
        },
        "success",
        `Refill ${copy.id} queued for ${rx.patient}`,
      );
    }
      ...
      const next = withAudit(
        {
          ...
          customers: a.customers ?? state.customers,
        ...
        "system",
        `Backup restored — ${a.products.length} products · ${a.transactions.length} receipts`,
      );
      return withToast(
        next,
        "success",
    ...
    case "RESTORE_EXPORT": {
      if (!validateOrgExport(a.bundle))
        return withToast(
          state,
          "error",
          "Invalid backup bundle — restore skipped",
        );
      ...
      const next = withAudit(
        {
          ...
          rxTransfers: data.rxTransfers,
          suppliers: data.suppliers,
          purchaseOrders: data.purchaseOrders,
          apInvoices: data.apInvoices,
          ...
          restrictedLog: data.restrictedLog,
          audit: data.audit,
          shifts: data.shifts,
          storeCredits: data.storeCredits,
          interactionPairs: data.interactionPairs,
          coldChainLog: data.coldChainLog,
        ...
        "system",
        `Org backup restored — ${a.bundle.organization_id} · ${new Date(a.bundle.exportedAt).toLocaleString()}`,
      );
      return withToast(
        next,
        "success",
        i18n.t("settings.backupRestored", {
          count: Object.keys(a.bundle.tables).length,
        }),
      );
    }
    ...
    case "REFUND_TX": {
      ...
      if (orig.refundOf)
        return withToast(state, "error", "Refund records can't be refunded");
      if (orig.refundedAt)
        return withToast(state, "warn", `${orig.id} was already refunded`);
      ...
      const products = state.products.map((p) => {
        ...
        if (line.alloc && line.alloc.length > 0)
          line.alloc.forEach((al) => give(al.batch, al.qty));
        else {
          const latest = [...batches].sort((x, y) =>
            y.expiry.localeCompare(x.expiry),
          )[0];
      ...
      const transactions = [
        ...
        ...state.transactions.map((t) =>
          t.id === orig.id ? { ...t, refundedAt: Date.now() } : t,
        ),
      ];
      let next = withAudit(
      ...
      return withToast(
        next,
        "success",
        `${orig.id} refunded — ${money(-orig.total)} returned, stock restored to lots`,
      );
    }
    ...
    case "ADD_PRODUCT":
      return withToast(
        withAudit(
          ...
          "stock",
          `New SKU — ${a.product.name} (${a.product.sku})`,
        ),
        "success",
      ...
      return withAudit(
        {
          ...
          products: state.products.map((x) =>
            x.id === a.productId
              ? { ...x, reorderLevel: Math.max(0, Math.round(a.reorderLevel)) }
              : x,
          ),
        },
        "stock",
        `Reorder level for ${target.name} set to ${Math.max(0, Math.round(a.reorderLevel))} (AI forecast, user-applied)`,
      );
    }
    ...
    case "RX_STATUS": {
      ...
      const msg =
        a.status === "verifying"
          ? `${rx.id} moved to pharmacist review`
          : a.status === "ready"
            ? `${rx.id} ready for pickup`
            : a.status === "dispensed"
              ? `${rx.id} dispensed — logged`
              : `${rx.id} reopened`;
      let next: State = {
        ...state,
        prescriptions: state.prescriptions.map((x) => {
          ...
          const dispensing =
            a.status === "dispensed" && x.status !== "dispensed";
          return {
            ...x,
            status: a.status,
            dispensedAt: dispensing
              ? (x.dispensedAt ?? Date.now())
              : x.dispensedAt,
            refillsRemaining:
              dispensing && x.refillsRemaining !== undefined
                ? Math.max(0, x.refillsRemaining - 1)
                : x.refillsRemaining,
          };
        }),
      ...
      if (a.status === "dispensed") {
        next = withAudit(
          next,
          "rx",
          `${rx.id} dispensed — ${rx.patient} · ${rx.qty} × ${rx.productId}${rx.refillsRemaining !== undefined ? ` · ${Math.max(0, rx.refillsRemaining - 1)} refill(s) left` : ""}`,
        );
      }
    ...
    case "RX_TO_CART": {
      ...
      if (avail <= 0)
        return withToast(
          state,
          "error",
          `${p.name} out of stock — cannot attach`,
        );
      ...
      const cart = existing
        ? state.cart.map((c) =>
            c.productId === p.id
              ? { ...c, qty: Math.min(c.qty + qty, avail) }
              : c,
          )
      ...
      return withToast(
        ...
        "info",
        `${rx.id} attached to register — ${qty} × ${p.name}`,
      );
    }
    ...
    case "SUBSTITUTE_GENERIC": {
      ...
      if (genericSubstituteFor(brand, state.products)?.id !== gen.id) {
        return withToast(
          state,
          "error",
          `${gen.name} is not a cheaper in-stock generic for ${brand.name}`,
        );
      }
      const same = (c: { productId: string; uom?: string }) =>
        c.productId === brand.id && (c.uom ?? "") === (a.uom ?? "");
      const line = state.cart.find(same);
      ...
      if (line.qty > maxUom)
        return withToast(
          state,
          "error",
          `Only ${maxUom} × ${gen.name} on the shelf — keeping ${brand.name}`,
        );
      /* merge onto an existing generic line if one is already on the ticket */
      const genSame = (c: { productId: string; uom?: string }) =>
        c.productId === gen.id && (c.uom ?? "") === (a.uom ?? "");
      const genLine = state.cart.find(genSame);
      const swapped = {
        ...line,
        productId: gen.id,
        daw: undefined,
        substitutedFrom: brand.id,
        ...
        ? state.cart
            ...
            .map((c) =>
              genSame(c)
                ? {
                    ...c,
                    qty: Math.min(c.qty + line.qty, maxUom),
                    substitutedFrom: brand.id,
                  }
                : c,
            )
    ...
    case "SET_DAW": {
      ...
      const cart = state.cart.map((c) =>
        same(c) ? { ...c, daw: code, substitutedFrom: undefined } : c,
      );
      return withToast(
        withAudit(
          { ...state, cart },
    ...
    case "CHARGE_RX_PICKUP": {
      ...
      if (rx.status === "dispensed")
        return withToast(
          state,
          "error",
          `${rx.id} already dispensed — cannot charge twice`,
        );
      const p = state.products.find((x) => x.id === rx.productId);
      if (!p)
        return withToast(
          state,
          "error",
          `Product for ${rx.id} is not in the catalog`,
        );
      const avail = stockOf(p, state.products);
      if (avail <= 0)
        return withToast(
          state,
          "error",
          `${p.name} out of stock — cannot charge ${rx.id} on pickup`,
        );
      const qty = Math.min(rx.qty, avail);
      ...
      const match = state.customers.find(
        (c) => c.name.toLowerCase() === rx.patient.toLowerCase(),
      );
      const customerId = state.saleCustomerId ?? match?.id ?? null;
      const same = (c: { productId: string; rxId?: string }) =>
        c.productId === p.id && c.rxId === rx.id;
      const cart = state.cart.some(same)
        ? state.cart.map((c) =>
            same(c) ? { ...c, qty: Math.min(qty, avail) } : c,
          )
      ...
      const next = withAudit(
        ...
        "rx",
        `${rx.id} queued for charge-on-pickup — ${qty} × ${p.name} · ${rx.patient}`,
      );
      return withToast(
        next,
        "info",
    ...
    case "PO_CREATE": {
      ...
      const po: PurchaseOrder = {
        ...
        status: "ordered",
        createdAt: Date.now(),
        expectedAt: Date.now() + sup.leadDays * 86_400_000,
      ...
      return withToast(
        ...
        "success",
        `${id} sent to ${sup.name} · ${money(total)}`,
      );
    }
    ...
    case "PO_RECEIVE": {
      ...
      const sup = po
        ? state.suppliers.find((s) => s.id === po.supplierId)
        : undefined;
      if (!po || !sup || po.status === "received" || po.status === "cancelled")
        return state;
      ...
      const receivedValue = round2(
        lines.reduce((s, l) => s + l.received * l.unitCost, 0) -
      ...
      const invoice: ApInvoice = {
        ...
        date: Date.now(),
        dueDays: sup.terms,
        total: Math.max(0, receivedValue),
        payments: [],
        credits: [],
      };
      const updated: PurchaseOrder = {
        ...
        lines,
        status,
        receivedAt: allIn ? Date.now() : po.receivedAt,
        invoiceId: invoice.total > 0 ? invNumber : po.invoiceId,
      };
      let next: State = {
        ...state,
        products,
        purchaseOrders: state.purchaseOrders.map((x) =>
          x.id === po.id ? updated : x,
        ),
        apInvoices:
          invoice.total > 0 ? [invoice, ...state.apInvoices] : state.apInvoices,
      };
      next = withAudit(
        next,
        "stock",
        `Received against ${po.id} (${sup.name}) — ${a.receipts.filter((r) => r.qty > 0).length} lot(s)${invoice.total > 0 ? ` · ${invNumber} ${money(invoice.total)} due net-${sup.terms}` : ""}`,
      );
      return withToast(
        next,
        "success",
        `${po.id} received — stock added${invoice.total > 0 ? `, ${invNumber} booked ${money(invoice.total)}` : ""}`,
      );
    }
        ...
        withAudit(
          {
            ...state,
            purchaseOrders: state.purchaseOrders.map((x) =>
              x.id === a.poId ? { ...x, status: "cancelled" } : x,
            ),
          },
          "stock",
          `PO ${a.poId} cancelled`,
    ...
    case "AP_PAY": {
      ...
      const apInvoices = state.apInvoices.map((x) =>
        x.id === a.invoiceId
          ? {
              ...x,
              payments: [
      ...
      return withToast(
        ...
        "success",
        nowPaid
          ? `${inv.number} settled in full`
          : `Paid ${money(amount)} on ${inv.number} · ${money(invoiceBalance(inv) - amount)} remaining`,
      );
    }
    ...
    case "AP_CREDIT": {
      ...
      return withToast(
        withAudit(
          { ...state, apInvoices },
          "money",
          `Credit note ${money(a.amount)} on ${inv.number} — ${a.note}`,
        ),
        "success",
        `Credit of ${money(a.amount)} applied to ${inv.number}`,
      );
    }
    ...
    case "EXPENSE_ADD": {
      ...
      const exp: Expense = {
        ...
        payee: a.payee.trim() || "—",
        note: a.note?.trim() || undefined,
        recurring: a.recurring || undefined,
      };
      return withToast(
        withAudit(
        ...
        "success",
        `${exp.category} expense recorded · ${money(exp.amount)}`,
      );
    }
    ...
    case "EXPENSE_DELETE":
      return {
        ...state,
        expenses: state.expenses.filter((x) => x.id !== a.id),
      };
    ...
    case "DELIVERY_STATUS": {
      ...
      const deliveries = state.deliveries.map((x) =>
        x.id === a.id
          ? {
              ...x,
              status: a.to,
              driver: a.driver !== undefined ? a.driver : x.driver,
              proof: a.proof !== undefined ? a.proof : x.proof,
            }
          : x,
      );
      const verb =
        a.to === "assigned"
          ? `assigned to ${a.driver ?? "driver"}`
          : a.to === "out"
            ? "out for delivery"
            : a.to === "delivered"
              ? "delivered ✓"
      ...
      return withToast(
        withAudit(
          { ...state, deliveries },
          "sale",
          `Delivery ${a.id} ${verb} — ${cust?.name ?? d.customerId}`,
        ),
        "success",
        `${a.id} ${verb}`,
      );
    ...
    case "WEB_ORDER": {
      ...
      if (!w) return state;
      const webOrders = state.webOrders.map((x) =>
        x.id === a.id
          ? {
              ...x,
              status: a.to,
      ...
      return withToast(
        withAudit(
          { ...state, webOrders },
          "sale",
          `Web order ${a.id} ${a.to} — ${w.customerName}`,
        ),
        a.to === "declined" ? "warn" : "success",
        `${a.id} ${a.to}`,
      );
    }
    ...
    case "WEB_CONVERT": {
      ...
      const cust = state.customers.find(
        (c) => c.name.toLowerCase() === w.customerName.toLowerCase(),
      );
      const id = `DL-${304 + state.deliveries.length}`;
      const delivery: Delivery = {
        id,
        customerId: cust?.id ?? "WALK-IN",
        address:
          w.pickup === "in_store"
            ? "In-store pickup"
            : (cust?.address ?? "Address on file"),
        lines: w.items
      ...
      const webOrders = state.webOrders.map((x) =>
        x.id === a.id ? { ...x, status: "converted" as const } : x,
      );
      return withToast(
        withAudit(
          { ...state, webOrders, deliveries: [delivery, ...state.deliveries] },
          "sale",
          `Web order ${a.id} → delivery ${id} (${w.pickup})`,
        ),
        "success",
        `${a.id} converted → ${id} queued`,
      );
    }
    ...
    case "CLOCK": {
      ...
      const open = state.timeEntries.find(
        (t) => t.staffId === state.user!.id && !t.outAt,
      );
      if (open) {
        const timeEntries = state.timeEntries.map((t) =>
          t.id === open.id ? { ...t, outAt: Date.now() } : t,
        );
        ...
        return withToast(
          withAudit(
            { ...state, timeEntries },
            "system",
            `${state.user.name} clocked out — ${hrs}h shift`,
          ),
          "info",
          `Clocked out — ${hrs}h on shift`,
        );
      }
      const entry: TimeEntry = {
        id: (state.timeEntries[0]?.id ?? 500) + 1,
    ...
    case "CUSTOMER_PROFILE": {
      ...
      return withToast(
        withAudit(
          { ...state, customers },
          "rx",
          `Patient profile updated — ${c.name}`,
        ),
        "success",
    ...
    case "HYDRATE_BACKEND": {
      const hydratedUser = state.user
        ? (a.data.staff.find(
            (staff) => staff.id === state.user?.id && staff.active,
          ) ?? state.user)
        : null;
    ...
    case "SHIFT_OPEN": {
      if (!state.user) return state;
      const terminalId =
        a.terminalId || `T-${String(state.shifts.length + 1).padStart(2, "0")}`;
      ...
      return withToast(
        withAudit(
          { ...state, shifts: [...state.shifts, shift], currentShift: shift },
          "shift",
          `Shift ${shift.id} opened by ${state.user.name}`,
        ),
        "success",
        `Shift opened — ${terminalId}`,
      );
    }
    ...
    case "SHIFT_CLOSE": {
      ...
      return withToast(
        withAudit(
          { ...state, shifts: updatedShifts, currentShift: null },
          "shift",
          `Shift ${closed.id} closed — over/short: ${closed.overShort?.toFixed(2)}`,
        ),
        "info",
        `Shift closed — ${closed.overShort! >= 0 ? "over" : "short"} $${Math.abs(closed.overShort!).toFixed(2)}`,
      );
    }
    ...
    case "SHIFT_CASH_MOVEMENT": {
      ...
      const needsApproval =
        a.amount > 100 && !managerRoles.includes(state.user.role);
      if (needsApproval && !a.approvedBy)
        return withToast(
          state,
          "error",
          "Manager approval required for large amounts",
        );

      const updated = recordCashMovement(
        state.currentShift,
        a.movementType,
        a.amount,
        a.reason,
      ...
      return withToast(
        withAudit(
          { ...state, shifts: updatedShifts, currentShift: updated },
          "cash",
          `${a.movementType.replace("_", " ")} $${a.amount.toFixed(2)} — ${a.reason}`,
        ),
        "success",
        `${a.movementType.replace("_", " ").toUpperCase()} recorded`,
      );
    }
    ...
    case "VOID_TX": {
      ...
      if (!can(state.user?.role, "refund"))
        return withToast(state, "error", "Manager approval required for voids");
      ...
      const updatedTransactions = state.transactions.map((t) =>
        t.id === a.txId
          ? {
              ...
              voidedAt: Date.now(),
              voidReason: a.reason,
              voidedBy: a.approvedBy || state.user?.name,
            }
          : t,
      );
      ...
      return withToast(
        withAudit(
          newState,
          "void",
          `Transaction ${a.txId} voided by ${state.user?.name ?? "unknown"} — ${a.reason}`,
        ),
        "warn",
        "Transaction voided",
      );
    ...
    case "GENERATE_X_REPORT": {
      ...
      return withToast(
        state,
        "info",
        `X Report generated for shift ${shift.id}`,
      );
    }
    ...
    case "GENERATE_Z_REPORT": {
      ...
      // In a real app, this would show a modal or navigate to a report view
      return withToast(
        withAudit(state, "report", `Z Report generated for shift ${shift.id}`),
        "success",
        `Z Report generated — ${report.overShort >= 0 ? "over" : "short"} $${Math.abs(report.overShort).toFixed(2)}`,
      );
    }
...
interface Ctx {
  ...
  /** Force a kept-local row through despite an LWW conflict (keep-local override). */
  forceOutbox: (table: string, key: string) => void;
}
...
export function PosProvider({ children }: { children: ReactNode }) {
  ...
  // Last persisted row snapshot per table; used to diff offline mutations into the outbox.
  const lastRowsRef = useRef<Record<string, Record<string, unknown>[]>>({});
  ...
  useEffect(() => {
    ...
    const hydrate = async () => {
      const result = await loadBackendData(
        backendDataFromState(stateRef.current),
      );
      if (!active) return;
      if (result.ok) {
        if (result.data.snapshots.length > 0)
          writeSnapshots(result.data.snapshots);
   ...
   * While offline we don't silently drop (the old warn path): every changed row is diffed
   * against the last synced snapshot and enqueued into the localStorage outbox instead. */
  useEffect(() => {
    if (
      !hydratedRef.current ||
    ...
    const rows = rowsFor(backendDataFromState(state));
    if (!state.online) {
      // Offline: accumulate row-level diffs into the outbox for replay on reconnect.
      const prevRows = lastRowsRef.current;
      for (const table of Object.keys(rows) as (keyof typeof rows)[]) {
        const next = rows[table] as Record<string, unknown>[];
        const prev = prevRows[table] ?? [];
        if (JSON.stringify(prev) === JSON.stringify(next)) continue;
        writeOutbox(enqueueChanges(table, "id", prev, next));
      }
      lastRowsRef.current = rows;
      dispatch({ type: "SET_OUTBOX_COUNT", count: readOutbox().length });
      return;
    }
    lastRowsRef.current = rows;
  ...
  /* Replay the offline outbox on reconnect (W3.3): LWW per row, then a full sync push. */
  const drainOutboxNow = useCallback(
    async (force?: { table: string; key: string }) => {
      if (drainingRef.current || readOutbox().length === 0) return;
      drainingRef.current = true;
      try {
        const result = await drainOutbox({
          force,
          fetchRow: async (table, key) => {
          ...
          writeRow: (table, payload) => upsertRow(table, payload),
        });
        if (result.conflicts.length)
          dispatch({ type: "ADD_CONFLICTS", conflicts: result.conflicts });
        if (result.remaining === 0) clearOutbox();
        dispatch({ type: "SET_OUTBOX_COUNT", count: readOutbox().length });
        // After the outbox is replayed, push the full local state so derived rows land too.
        if (hydratedRef.current && stateRef.current.backendAuthenticated) {
          void persistBackendData(backendDataFromState(stateRef.current));
        }
      } finally {
  ...
  useEffect(() => {
    const on = () => {
      ...
      // Retry persist on reconnect: replay the offline outbox, then push any queued edits.
      if (hydratedRef.current && stateRef.current.backendAuthenticated) {
        void drainOutboxNow();
      }
    };
  ...
  const value = useMemo<Ctx>(() => {
    ...
    const prescriber = (id: string) =>
      state.prescribers.find((p) => p.id === id);
    ...
    const expiring = state.products
      .filter((p) => {
        ...
        return e !== null && daysUntil(e) <= 60;
      })
      .sort((a, b) =>
        (nearestExpiry(a) ?? "").localeCompare(nearestExpiry(b) ?? ""),
      );
    const newRx = state.prescriptions.filter(
      (r) => r.status === "new" || r.status === "verifying",
    ).length;
    ...
    const items = sales.reduce(
      (s, t) => s + t.lines.reduce((x, l) => x + l.qty, 0),
      0,
    );
    ...
    return {
      ...
      todayStats: {
        revenue,
        count: sales.length,
        avg: sales.length ? round2(revenue / sales.length) : 0,
        items,
      },
      money,
      forceOutbox: (table: string, key: string) => {
        dispatch({ type: "KEEP_LOCAL_CONFLICT", table, key });
        void drainOutboxNow({ table, key });
      },
    };
  }, [state]);
